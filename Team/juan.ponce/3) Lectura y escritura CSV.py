...
# MAGIC %md
# MAGIC ## Lectura y escritura en CSV
# MAGIC CSV es un formato de archivo de texto sin formato que almacena datos tabulares. Cada fila representa un registro y los campos están separados por comas.
# MAGIC
# MAGIC En este cuaderno, crearé un DataFrame con información del país y luego les mostraré cómo escribirlo en el FileStore de Databricks en formato CSV y leerlo.
...
# MAGIC %md
# MAGIC ### Escribir en formato CSV

# COMMAND ----------

# MAGIC %md
# MAGIC Creación de un DataFrame de muestra
...
# MAGIC %md
# MAGIC Aquí, escribimos el DataFrame en el FileStore. 
# MAGIC En lugar de escribir un único archivo CSV llamado `countries_csv`, Spark crea un directorio con ese nombre que contiene varios archivos particionados.
# MAGIC
# MAGIC Ignore los archivos con guion bajo (como `_SUCCESS`, `_committed`, `_started`), ya que son archivos marcadores que indican el estado del trabajo.

# COMMAND ----------

#df.write.csv("dbfs:/FileStore/write_demo/countries_csv")
df.write.csv("abfss://data@dbstoragezat11.dfs.core.windows.net/csv/countries_csv",mode="overwrite")

# COMMAND ----------

# MAGIC %md
# MAGIC Después de escribir, inspeccionemos la estructura del directorio usando dbutils:

# COMMAND ----------

#display(dbutils.fs.ls("abfss://inputs@dbstoragezat10.dfs.core.windows.net/exportcsv/"))
display(dbutils.fs.ls("abfss://inputs@dbstoragezat11.dfs.core.windows.net/exportcsv/countries_csv"))

# COMMAND ----------

# MAGIC %md
# MAGIC ### Visualización de un archivo de partición
# MAGIC
# MAGIC Veamos uno de los archivos de partición. El nombre del archivo empieza por "part-" seguido de un identificador único. Cada archivo representa una partición del conjunto de datos completo.

# COMMAND ----------

print(dbutils.fs.head('abfss://inputs@dbstoragezat10.dfs.core.windows.net/exportcsv/countries_csv/part-00000-tid-3202908405530295671-c11057e7-5205-41e2-90b8-e22da5cb88bc-0-1-c000.csv', 10000))

# COMMAND ----------

# MAGIC %md
# MAGIC Para sobrescribir los datos existentes, podemos especificar el modo como "sobrescribir":

# COMMAND ----------

df.write.csv("abfss://inputs@dbstoragezat10.dfs.core.windows.net/exportcsv/countries_csv", mode="overwrite")

# COMMAND ----------

# MAGIC %md
# MAGIC También podemos especificar el modo mediante el método mode(). Tenga en cuenta que el modo debe especificarse antes de guardar:

# COMMAND ----------

df.write.mode("overwrite").csv("abfss://inputs@dbstoragezat10.dfs.core.windows.net/exportcsv/countries_csv2")
...
# MAGIC %md
# MAGIC Prefiero usar el método `save` porque permite especificar el formato de archivo dinámicamente. Aquí está la sintaxis equivalente:

# COMMAND ----------

df.coalesce(1).write.format("csv").mode("overwrite").save("abfss://inputs@dbstoragezat10.dfs.core.windows.net/exportcsv/countries_csv3")

# COMMAND ----------

# MAGIC %md
# MAGIC ## Especificación de opciones CSV
# MAGIC
# MAGIC Podemos personalizar la salida CSV especificando opciones como el separador y si se debe incluir un encabezado. A continuación, se muestran dos maneras de hacerlo.
...
df.write.format("csv") \
    ...
    .option("header", True) \
    .mode("overwrite") \
    .save("abfss://inputs@dbstoragezat10.dfs.core.windows.net/exportcsv/countries_csv4")

# Using the options method to set multiple options at once
df.write.format("csv") \
    .options(sep="[", header=True) \
    .mode("overwrite") \
    .save("abfss://data@dbstoragezat11.dfs.core.windows.net/csv/countries_csv3")
 

# COMMAND ----------

# MAGIC %md
# MAGIC Veamos los datos con dbutils. Primero, listamos los archivos del directorio:

# COMMAND ----------

display(dbutils.fs.ls("abfss://inputs@dbstoragezat10.dfs.core.windows.net/exportcsv/countries_csv4"))

# COMMAND ----------

# MAGIC %md
# MAGIC Ahora, seleccione un archivo y muestre su contenido sin procesar:

# COMMAND ----------

print(dbutils.fs.head('abfss://inputs@dbstoragezat10.dfs.core.windows.net/exportcsv/countries_csv4/part-00003-tid-6334573599136690807-3d89c4e0-1913-4316-ba5b-000e34648d85-20-1-c000.csv', 10000))

# COMMAND ----------

# MAGIC %md
# MAGIC Observe cómo los valores ahora están separados por un corchete.

# COMMAND ----------

# MAGIC %md
# MAGIC ## Lectura de archivos CSV
# MAGIC
# MAGIC Ahora que tenemos datos CSV en Azure, los leeremos de nuevo en un DataFrame.
# MAGIC
# MAGIC Por defecto, Spark espera valores separados por comas. Dado que nuestro archivo usa punto y coma, debemos especificar las opciones de separador y encabezado.

# COMMAND ----------

# Reading without specifying options (not recommended for our file)
df_read_default = spark.read.csv("abfss://inputs@dbstoragezat10.dfs.core.windows.net/exportcsv/countries_csv4")
df_read_default.display()

# COMMAND ----------

# MAGIC %md
# MAGIC Lea el CSV correctamente especificando las opciones de separador y encabezado:

# COMMAND ----------

df_read = spark.read.csv("abfss://inputs@dbstoragezat10.dfs.core.windows.net/exportcsv/countries_csv4", sep="[", header=True)
df_read.display()

# COMMAND ----------

# MAGIC %md
# MAGIC Alternatively, we can use the `options` method:

# COMMAND ----------

df_read = spark.read.options(sep="[", header=True).csv("abfss://inputs@dbstoragezat10.dfs.core.windows.net/exportcsv/countries_csv4")
df_read.display()

# COMMAND ----------

# MAGIC %md
# MAGIC Revisemos el esquema del DataFrame. Observe que los archivos CSV almacenan los datos como texto sin formato, por lo que todos los valores se leen como cadenas por defecto.
...
# MAGIC %md
# MAGIC Para asignar tipos de datos apropiados, podemos inferir el esquema o especificarlo explícitamente.

# COMMAND ----------

# Infer schema automatically
df_inferred = spark.read.options(sep="[", header=True, inferSchema=True).csv("abfss://inputs@dbstoragezat10.dfs.core.windows.net/exportcsv/countries_csv4")
df_inferred.printSchema()

# COMMAND ----------

# MAGIC %md
# MAGIC O podemos especificar el esquema explícitamente:
