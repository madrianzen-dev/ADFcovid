...
def my_checkpoint_dir():
    return "tmp/delta_demo_prz/chkpt/%s" % str(random.randint(0, 10000))
...
def stop_all_streams():
    ...
    dbutils.fs.rm("tmp/delta_demo_prz/chkpt/",True)

#funcion para limpiar las rutas y tablas totales.
def limpia_paths_tablas():
    dbutils.fs.rm("tmp/delta_demo_prz/chkpt/",True)
    #dbutils.fs.rm("file:/dbfs/tmp/delta_demo_prz/loans_parquet/",True)

    #Voy a recorrer la tabla donde voy a limpiar los archivos
    for table in ["deltaZAT_db_PRZ.loans_parquet","deltaZAT_db_PRZ.loans_delta","deltaZAT_db_PRZ.loans_delta2"]:
...
# MAGIC %sh mkdir -p /dbfs/tmp/delta_demo_prz/loans_parquet/; wget -O /dbfs/tmp/delta_demo_prz/loans_parquet/loans.parquet https://pages.databricks.com/rs/094-YMS-629/images/SAISEU19-loan-risks.snappy.parquet

# COMMAND ----------

# MAGIC %sql
# MAGIC CREATE TABLE loans_delta_prz
# MAGIC using DELTA
# MAGIC as 
# MAGIC select * from parquet.`/tmp/delta_demo_prz/loans_parquet/`

# COMMAND ----------

#Estamos cargando un archivo parquet
parquet_path = "file:/dbfs/tmp/delta_demo_prz/loans_parquet/"
df = (spark.read.format("parquet").load(parquet_path)
      .withColumn("type",lit("batch"))
      .withColumn("timestamp",current_timestamp()))

#Lo vamos a escribir como formato delta
df.write.format("delta").mode("overwrite").saveAsTable("loans_delta_prz2")

# COMMAND ----------

# MAGIC %sql
# MAGIC convert to delta parquet.`/tmp/delta_demo_prz/loans_parquet/`

# COMMAND ----------

spark.sql("select count(*) from loans_delta_prz2").show()
spark.sql("select * from loans_delta_prz2").show(4)

# COMMAND ----------

#Vamos a crear escrituras en streaming de las tablas
stream_query_A = genera_data_streaming(table_format="delta",table_name="loans_delta_prz2",schema_ok=True,type='stream A')
stream_query_B = genera_data_streaming(table_format="delta",table_name="loans_delta_prz2",schema_ok=True,type='stream B')

# COMMAND ----------

display(spark.readStream.format("delta").table("loans_delta_prz2").groupBy("type").count().orderBy("type"))