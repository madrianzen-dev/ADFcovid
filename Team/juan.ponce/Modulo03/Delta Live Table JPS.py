...
def my_checkpoint_dir():
    return "tmp/delta_demo_jps/chkpt/%s" % str(random.randint(0, 10000))
...
def stop_all_streams():
    ...
    dbutils.fs.rm("tmp/delta_demo_jps/chkpt/",True)
...
def limpia_paths_tablas():
    ...
    for table in ["deltaZAT_db_MA.loans_parquet","deltaZAT_db_MA.loans_delta","deltaZAT_db_MA.loans_delta2"]:
...
# MAGIC %sh mkdir -p /dbfs/tmp/delta_demo_jps/loans_parquet/; wget -O /dbfs/tmp/delta_demo_jps/loans_parquet/loans.parquet https://pages.databricks.com/rs/094-YMS-629/images/SAISEU19-loan-risks.snappy.parquet

# COMMAND ----------

# MAGIC %sql
# MAGIC CREATE TABLE loans_delta2_jps
# MAGIC using DELTA
# MAGIC as 
# MAGIC select * from parquet.`/tmp/delta_demo_jps/loans_parquet/`

# COMMAND ----------

#Estamos cargando un archivo parquet
parquet_path = "file:/dbfs/tmp/delta_demo_jps/loans_parquet/"
...
# MAGIC %sql
# MAGIC convert to delta parquet.`/tmp/delta_demo_jps/loans_parquet/`
