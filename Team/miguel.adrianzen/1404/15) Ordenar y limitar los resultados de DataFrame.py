...
# MAGIC %md
# MAGIC ## Ordenar y limitar los resultados de un DataFrame
# MAGIC Ordenar un DataFrame con `sort()` o `limit()` permite reordenar las filas según una o más columnas (en orden ascendente o descendente).
# MAGIC
# MAGIC El método `limit()` restringe los resultados a un número específico de filas para una inspección rápida.
...
df.sort("population", ascending=False).limit(5).display()

# COMMAND ----------

# Sorting by continent and population in ascending order
df.sort("continent", "population", ascending=True).display()
