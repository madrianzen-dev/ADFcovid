...
-- Recreate the streaming table
CREATE OR REFRESH STREAMING TABLE taxi_raw_records2 (
  CONSTRAINT valid_distance EXPECT (trip_distance > 0.0) ON VIOLATION DROP ROW
) AS
SELECT
  *
FROM
  STREAM(zatdev111db.bronze.student_copy)

-- COMMAND ----------

CREATE OR REFRESH LIVE TABLE total_fare_amount_by_week AS
SELECT
  date_trunc("week", tpep_pickup_datetime) as week,
  SUM(fare_amount) as total_amount
FROM
  live.taxi_raw_records2
GROUP BY
  week
