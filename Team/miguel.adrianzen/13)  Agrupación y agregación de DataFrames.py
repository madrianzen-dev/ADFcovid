...
df.groupBy("continent").sum("population").sum("sub_region").display()

# COMMAND ----------

# sum() devuelve un DataFrame, por lo que no puede encadenar otro agregador como avg() en él después

df.groupBy("continent").agg({'population': 'sum','population': 'min', 'area_km2': 'avg'}).display()

# COMMAND ----------

# El uso de .agg() nos permite realizar múltiples funciones de agregación en un solo paso
from pyspark.sql.functions import col, round, sum, avg,min
#df.show(5)
df.groupBy("continent","sub_region").agg(
                                sum("population"),
                                avg("population")
                            ).display()
df2=df.groupBy("continent","sub_region").agg(
                                sum("population"),
                                avg("population"),
                                min("population")
                            ).select("continent", "sub_region",
                                     col("avg(population)").alias("avg_population"),
                                     col("sum(population)").alias("total_population"),
                                     col("min(population)").alias("min_population")
                                        )
df2.select("continent", "sub_region",
           round(df2["avg_population"],2).alias("avg_population_round2"),
           round(df2["total_population"],2).alias("total_population_round2"),
           round(df2["min_population"],2).alias("min_population_round2")
           ).display()                                             
