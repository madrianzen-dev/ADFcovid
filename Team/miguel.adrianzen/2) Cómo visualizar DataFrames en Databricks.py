...
# MAGIC %md
# MAGIC ## Cómo visualizar DataFrames en Databricks

# COMMAND ----------

# MAGIC %md
# MAGIC ### Enlaces y recursos
# MAGIC - [show()](https://spark.apache.org/docs/latest/api/python/reference/pyspark.sql/api/pyspark.sql.DataFrame.show.html#pyspark.sql.DataFrame.show)

# COMMAND ----------

# MAGIC %md
# MAGIC Hay dos maneras de previsualizar el contenido de un dataframe en Databricks:
# MAGIC
# MAGIC `show()` y `display()`.
# MAGIC
# MAGIC `show()` es un método integrado de PySpark DataFrame que imprime el DataFrame en formato de línea de comandos basado en texto.
# MAGIC
# MAGIC `display()` es una función específica de Databricks que ofrece una vista mucho más completa e interactiva del DataFrame.
# MAGIC
# MAGIC Esta función proporciona una tabla dinámica basada en web donde se puede ordenar, filtrar e incluso crear visualizaciones sobre la marcha.
# MAGIC
# MAGIC Sin embargo, es importante tener en cuenta que `display()` es exclusivo de los notebooks de Databricks y no funciona en entornos estándar de PySpark fuera de Databricks.
# MAGIC
# MAGIC
...
# MAGIC %md
# MAGIC ### Creating a sample DataFrame
...
# Displaying the DataFrame using the show method
df.show(200)
...
# Truncating values with over 2 characters
df.show(truncate=2)
