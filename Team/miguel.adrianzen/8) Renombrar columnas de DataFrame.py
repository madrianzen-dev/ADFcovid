...
# MAGIC %md
# MAGIC ## Renombrar columnas de DataFrame
# MAGIC Renombrar columnas en PySpark mejora la legibilidad de DataFrame y garantiza que los nombres de las columnas coincidan con las convenciones de nomenclatura deseadas.
# MAGIC
# MAGIC Puede usar métodos como withColumnRenamed() para renombrar una sola columna o withColumnsRenamed() para renombrar varias columnas a la vez, lo que hace que su código sea más intuitivo y esté alineado con la semántica de sus datos.
...
# Creación de un objeto de columna con la función col y cambio de nombre de la columna country_name a country
df.select(col("country_name").alias("countryF"), col("population")).show(5)

# Creación de un objeto de columna con acceso a atributos de DataFrame y cambio de nombre de la columna country_name a country
df.select(df.country_name.alias("country"), df.population).show(5)

# Creación de un objeto de columna con notación de corchetes y cambio de nombre de la columna country_name a country
df.select(df['country_name'].alias("country"), df['population']).show(5)

# COMMAND ----------

# using the withColumnRenamed method to rename the country_name column to country
df.withColumnRenamed("country_name", "country2").show()#.select("country2","population").show(5)

# COMMAND ----------

# Chaining multiple withColumnRenamed calls
df.\
...
# Rename multiple columns in one step using withColumnsRenamed:
# - "country_name" is renamed to "country"
# - "population" is renamed to "country_population"
df.\
    withColumnsRenamed({"country_name":"country", "population":"country_population"}).show(5)

# COMMAND ----------

# Since PySpark DataFrames are immutable, transformations like `withColumn()` return a new DataFrame. We reassign the result to a new variable (`df_2`) to retain the changes.
df_2 = df.\
    withColumnsRenamed({"country_name":"country", "population":"country_population"})
