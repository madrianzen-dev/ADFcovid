...
# MAGIC %md
# MAGIC ## Filtrado de un DataFrame
# MAGIC En PySpark, el filtrado se realiza mediante los métodos `filter()` o `where()` para conservar las filas que cumplen una condición. Admite expresiones de columna y cadenas de estilo SQL.
# MAGIC
# MAGIC
# MAGIC
...
df.filter("continent = 'Asia' and country_name = 'India'").select(df['country_name']).display()

# COMMAND ----------

# Filtrar filas donde la densidad de población (población dividida por área en km²) es mayor a 1000
