...
# MAGIC %md
# MAGIC ##Introducción a las transformaciones de columnas
# MAGIC
# MAGIC Puede agregar nuevas columnas, modificar las existentes y aplicar funciones mediante los métodos withColumn() y select() para preparar sus datos para el análisis.
...
# MAGIC %md
# MAGIC ### Método withColumn()
# MAGIC
# MAGIC En esta sección, mostramos cómo agregar una nueva columna mediante el método `withColumn()`. La nueva columna `population_m` representa la población en millones.

# COMMAND ----------

from pyspark.sql.functions import col

# Add a new column 'population_m' by dividing 'population' by 1,000,000
df.withColumn("population_m", col("population")/1000000).show(5)

# COMMAND ----------

# MAGIC %md
# MAGIC Dado que los DataFrames de PySpark son inmutables, el método `withColumn()` devuelve un nuevo DataFrame con la transformación aplicada. Aquí, asignamos el DataFrame transformado a una nueva variable `df_2` para conservar los cambios.

# COMMAND ----------

# Reassign the transformed DataFrame to a new variable
df_2 = df.withColumn("population_m", col("population")/1000000)

# Display the original DataFrame (df) and the transformed DataFrame (df_2)
df.show(5)
df_2.show(5)
...
# MAGIC %md
# MAGIC También podemos transformar una columna existente. En este ejemplo, convertimos la columna `country_name` a mayúsculas mediante la función `upper()`.
...
df.withColumn("country_name", upper("country_name")).show(5)

# COMMAND ----------

# MAGIC %md
# MAGIC ### Encadenamiento de múltiples transformaciones con withColumn()
# MAGIC Aquí, encadenamos múltiples llamadas a withColumn() para aplicar varias transformaciones en secuencia. Cada llamada crea un nuevo DataFrame, lo que puede ser menos eficiente para muchas transformaciones.
# MAGIC

# COMMAND ----------

df.withColumn("country_name", upper("country_name"))\
  .withColumn("population_m", col("population")/1000000)\
  .show(5)

# COMMAND ----------

# MAGIC %md
# MAGIC ### Aplicación de múltiples transformaciones con select()
# MAGIC
# MAGIC Usando el método `select()`, podemos aplicar múltiples transformaciones en un solo paso, lo cual suele ser más eficiente que encadenar varias llamadas `withColumn()`. En este ejemplo:
# MAGIC - Convertimos `country_name` a mayúsculas y le asignamos el alias `country_name`.
# MAGIC - Mantenemos la columna `continent`.
# MAGIC - Creamos la nueva columna `population_m` dividiendo `population` entre 1 000 000.
# MAGIC - Mantenemos la columna `area_km²`.

# COMMAND ----------

df.select(
    upper("country_name").alias("country_name"),  # Convertir los nombres de los países a mayúsculas
    "continent", 
    "population",                                 # Conservar la columna 'continente'
    (col("population")/1000000).alias("population_m"),  # Crear la columna 'population_m'
    "area_km2"                                    # Conservar la columna 'area_km2'
).show(5)