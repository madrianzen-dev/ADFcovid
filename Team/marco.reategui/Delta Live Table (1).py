...
def my_checkpoint_dir():
    return "tmp/delta_demo_march/chkpt/%s" % str(random.randint(0, 10000))
...
def stop_all_streams():
    ...
    dbutils.fs.rm("tmp/delta_demo_march/chkpt/",True)
...
def limpia_paths_tablas():
    ...
    for table in ["deltaZAT_db_MARCH.loans_parquet","deltaZAT_db_MARCH.loans_delta","deltaZAT_db_MA.loans_delta2"]:
...
# MAGIC %sh mkdir -p /dbfs/tmp/delta_demo_march/loans_parquet/; wget -O /dbfs/tmp/delta_demo_march/loans_parquet/loans.parquet https://pages.databricks.com/rs/094-YMS-629/images/SAISEU19-loan-risks.snappy.parquet

# COMMAND ----------

#Estamos cargando un archivo parquet
parquet_path = "file:/dbfs/tmp/delta_demo_march/loans_parquet/"
...
# MAGIC using DELTA
# MAGIC as 
# MAGIC select * from parquet.`/tmp/delta_demo_march/loans_parquet/`

# COMMAND ----------

# MAGIC %sql
# MAGIC convert to delta parquet.`/tmp/delta_demo_march/loans_parquet/`
# MAGIC     
