...
db = "zatdev111db.deltaZAT_db_HRC"
...
def my_checkpoint_dir():
    return "tmp/delta_demo/chkpt_hrc/%s" % str(random.randint(0, 10000))
...
def stop_all_streams():
    ...
    dbutils.fs.rm("tmp/delta_demo/chkpt_hrc/",True)
...
def limpia_paths_tablas():
    ...
    for table in ["zatdev111db.deltaZAT_db_HRC.loans_parquet","zatdev111db.deltaZAT_db_HRC.loans_delta","zatdev111db.deltaZAT_db_HRC.loans_delta2"]:
...
# MAGIC %sql
# MAGIC CREATE TABLE zatdev111db.deltaZAT_db_HRC.loans_delta_hrc
# MAGIC using DELTA
# MAGIC as 
...
spark.sql("select count(*) from zatdev111db.deltaZAT_db_HRC.loans_delta_hrc").show()
spark.sql("select * from zatdev111db.deltaZAT_db_HRC.loans_delta_hrc").show(4)

# COMMAND ----------

#Vamos a crear escrituras en streaming de las tablas
stream_query_A = genera_data_streaming(table_format="delta",table_name="zatdev111db.deltaZAT_db_HRC.loans_delta_hrc_A",schema_ok=True,type='stream A')
stream_query_B = genera_data_streaming(table_format="delta",table_name="zatdev111db.deltaZAT_db_HRC.loans_delta_hrc_B",schema_ok=True,type='stream B')



# COMMAND ----------

display(spark.readStream.format("delta").table("zatdev111db.deltaZAT_db_HRC.loans_delta_hrc_A").groupBy("type").count().orderBy("type"))