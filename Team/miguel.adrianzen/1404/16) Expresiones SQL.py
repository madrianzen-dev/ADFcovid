...
# MAGIC `expr()`: Función que convierte una cadena de expresión SQL en una columna Spark.
# MAGIC
# MAGIC `selectExpr()`: Método DataFrame que selecciona y transforma columnas usando cadenas de expresión SQL simultáneamente.
...
# MAGIC - [selectExpr()](https://spark.apache.org/docs/latest/api/python/reference/pyspark.sql/api/pyspark.sql.DataFrame.selectExpr.html?highlight=selectexpr#pyspark.sql.DataFrame.selectExpr)
...
df.select("country_name", expr("upper(country_name) as country_name_upper")).limit(10).display()

# This uses the upper function from the functions module in Spark SQL
df.select("country_name", upper("country_name").alias("country_name_upper")).limit(10).display()

# COMMAND ----------

# expr() can be used to combine multiple SQL functions and alias column names

df.select("country_name","continent", expr("round(population/area_km2,1) as population_density_km2")).limit(4).display()
...
df.selectExpr("country_name","population","area_km2","round(population/area_km2,2) as population_density_km2").display()