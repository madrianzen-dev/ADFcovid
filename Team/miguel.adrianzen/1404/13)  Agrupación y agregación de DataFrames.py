...
from pyspark.sql.functions import col, sum, avg, asc, min,desc# El uso de .agg() nos permite realizar múltiples funciones de agregación en un solo paso

df2=df.groupBy("continent", "sub_region").agg(
                                sum("population"),
                                avg("population")
                            ).select("continent", "sub_region",
                                      col("sum(population)").alias("country"),
                                      col("avg(population)").alias("promedio poblacion")
                                        )#.display()
                            
df2.select("continent", "sub_region",
           func.round(df2["country"], 2).alias("dd"),
           func.round(df2["promedio poblacion"], 2).alias("dd3")
          ).sort(asc("continent"),desc("sub_region")).display() 
...
df.groupBy("continent").agg(
                                ...
                                min("area_km2").alias("total_area_km2")
                            ).display()
