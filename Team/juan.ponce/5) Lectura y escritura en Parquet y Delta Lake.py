...
# MAGIC
# MAGIC `Apache Parquet` es un formato de almacenamiento en columnas que organiza los datos por columnas, lo que permite una compresión eficiente y un rendimiento de consultas rápido, ya que solo se leen las columnas necesarias durante el análisis. Su diseño es ideal para gestionar cargas de trabajo analíticas a gran escala, lo que lo convierte en una opción común en entornos de big data.
# MAGIC
# MAGIC `Delta Lake` se basa en Parquet, añadiendo capacidades transaccionales y una gestión mejorada de metadatos. Los datos se almacenan como archivos Parquet, con un registro de transacciones (_delta_log) que registra cada cambio y habilita las transacciones ACID (Atomicidad, Consistencia, Aislamiento y Durabilidad), lo que garantiza la completa finalización de las operaciones, la consistencia de los datos, la ejecución aislada de las transacciones y la persistencia fiable de los cambios.
# MAGIC
# MAGIC `Delta Lake` también implementa esquemas en evolución, admite la consulta de versiones anteriores de datos mediante el uso de la función de viaje en el tiempo y gestiona eficientemente las actualizaciones incrementales para cargas de trabajo por lotes y en streaming.
...
# MAGIC - [Parquet vs Delta Lake Comparison](https://delta.io/blog/delta-lake-vs-parquet-comparison/)
...
# Escritura de DataFrame en formato Parquet

# Using parquet method
#df.write.mode("overwrite").parquet("abfss://inputs@dbstoragezat10.dfs.core.windows.net/write_demo/countries_parquet5")
df.write.mode("overwrite").parquet("abfss://data@dbstoragezat11.dfs.core.windows.net/parquet/countries_parquet")

# Using save method
df.coalesce(1).write.format("parquet").mode("overwrite").save("abfss://inputs@dbstoragezat10.dfs.core.windows.net/write_demo/countries_parquet6")

# COMMAND ----------

# Escritura de DataFrame en formato Delta Lake
# Delta Lake solo admite el método de guardado
df.write.format("delta").save("abfss://inputs@dbstoragezat10.dfs.core.windows.net/write_demo/countries_delta")

# COMMAND ----------

# Lectura y visualización de archivos de Parquet y Delta Lake
spark.read.format("parquet").load("abfss://inputs@dbstoragezat10.dfs.core.windows.net/write_demo/countries_parquet5").display()
spark.read.format("delta").load("abfss://inputs@dbstoragezat10.dfs.core.windows.net/write_demo/countries_delta").display()