...
# MAGIC %md
# MAGIC ## Expresiones condicionales
# MAGIC
# MAGIC `expr()` permite escribir expresiones condicionales en línea similares a SQL.
# MAGIC
# MAGIC `when()`, combinado con `otherwise()`, proporciona una forma programática de aplicar lógica condicional en transformaciones de DataFrame.
...
df.select("country_name", 
          ...
          "population", 
          "area_km2", 
          expr("case when population > 300000000 then 'High' when population > 20000000 then 'Medium' else 'Small' end").alias("population_class")).limit(4).\
display()
